...
RUN set -x && \
    ...
    apt-get update && \
    apt-get install -y chromium-browser chromium-chromedriver php7.2-fpm php7.2-mysql php7.2-sqlite3 php7.2-mbstring php7.2-json php7.2-zip php7.2-gd php7.2-bz2 php7.2-opcache supervisor cron rsyslog libhyphen0 make ssh git && \
    apt-get clean -y && \
    mkdir -p /run/php && \
    mkdir -p /home/node -p && \
